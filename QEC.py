    ...
    def construct_circuit_stabilizer(self,stabilizer:str,stabilizer_index:int):
        ...
        for index in range(0,len(stabilizer)):
            ...
            if stabilizer[index]=="Z":
                self._circuit.cx(index,self._num_physical_qubits+stabilizer_index)
            #Add X stabilizer check    
            elif stabilizer[index]=="X":
                self._circuit.h(self._num_physical_qubits+stabilizer_index)
                self._circuit.cx(self._num_physical_qubits+stabilizer_index,index)
                self._circuit.h(self._num_physical_qubits+stabilizer_index)
...
class fourqubitcode(QECCode):
    

    def __init__(self, physical_qubits=4):
        super().__init__(physical_qubits,2,2)
        self.set_stabilizers(["XXXX","ZZZZ"]) 
        self._mutecorrection=True
        
    #Initial state is |00>_L=|0000>+|1111>    
    def construct_encoding_circuit(self):
        self._circuit.h(0)
